...
class Ant(Insect):
    ...
    def can_contain(self, insect):
        if ((self.container == True) and (self.ant == None) and (insect.container == False)):
            return True
        else:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        i = 0
        while (test_place.entrance.name != 'Hive' and len(test_place.bees) == 0) or i < self.min_range:
            i += 1
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        bee = self.place.bees
        if self.digesting == 0:
            return self.eat_bee(random_or_none(bee))
        else:
            self.digesting -= 1
        # END Problem 6B
...
class BodyguardAnt(Ant):
    ...
    def contain_ant(self, ant):
        # BEGIN Problem 7
        if Ant.can_contain(self, ant) == True:
            self.ant = ant 
...
class QueenAnt(Ant):  # You should change this line
    ...
    def double(self, ant):
        if ant == None:
            return
        if (ant in self.doubled) == False:
            ant.damage *= 2
            self.doubled.append(ant)
        if ant.container:
            inside = ant.ant
            if inside and (inside in self.doubled) == False:
                inside.damage *= 2
