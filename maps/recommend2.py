...
def find_centroid(cluster):
    ...
    for restaurant in cluster:
        loc1 = restaurant_location(restaurant)

        lat1 = loc1[0]
        long1 = loc1[1]

        avg_lat.append(lat1)
        avg_long.append(long1)

    mean_lat = mean(avg_lat)
    mean_long = mean(avg_long)
...
def find_predictor(user, restaurants, feature_fn):
    ...
    sxy = sum([(zs[i][0] - mean_xs)*(zs[i][1] - mean_ys) for i in range(len(zs))])
    ...
    b, a, r_squared = sxy/sxx, mean_ys - (sxy/sxx)*mean_xs, (sxy**2)/(sxx*syy)  # REPLACE THIS LINE WITH YOUR SOLUTION
    # END Question 7
...
def best_predictor(user, restaurants, feature_fns):
    ...
    x = [find_predictor(user, reviewed, f) for f in feature_fns]
