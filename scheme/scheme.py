...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    def check_and(expressions):
        if expressions is nil:
            return True
        elif scheme_falsep(expressions.first):
            return False
        elif scheme_truep(expressions.first) and (expressions.second is nil):
            
            return scheme_eval(expressions.first, env)
        elif scheme_truep(expressions.first) and (expressions.second is not nil):
            if scheme_falsep(scheme_eval(expressions.first, env)):
                return False
            return check_and(expressions.second)

    return check_and(expressions)
    # END PROBLEM 13

def do_or_form(expressions, env):
    """Evaluate a short-circuited or form."""
    # BEGIN PROBLEM 13
    def check_or(expressions):
        if expressions is nil:
            return False

        elif scheme_truep(expressions.first):
            if scheme_falsep(scheme_eval(expressions.first, env)):
                return check_or(expressions.second)
            return scheme_eval(expressions.first, env)
        elif scheme_falsep(expressions.first) and (expressions.second is not nil):
            return check_or(expressions.second)
        else:
            return False
       
    
    return check_or(expressions)
    # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if not scheme_symbolp(first_pair.first):
            raise SchemeError
        
        formal_symbol = first_pair.first
        evaluated = scheme_eval(first_pair.second.first, env)

        formals = Pair(formal_symbol, formals)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    if tail:
        # BEGIN Extra Credit
        return scheme_optimized_eval(scheme_evl(expr, env))
        # END Extra Credit
    else:
